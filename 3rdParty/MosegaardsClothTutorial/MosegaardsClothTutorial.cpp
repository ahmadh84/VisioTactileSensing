...
public:
	Particle(Vec3 pos) :
			pos(pos), old_pos(pos), acceleration(Vec3(0, 0, 0)), mass(1), movable(true), accumulated_normal(Vec3(0, 0, 0)) {
	}
	Particle() {
	}
...
private:
	...
	void drawTriangle(Particle *p1, Particle *p2, Particle *p3, const Vec3 color) {
		...
		Vec3* n1 = new Vec3(p1->getNormal().normalized());
		glNormal3fv((GLfloat *) n1);
		glVertex3fv((GLfloat *) &(p1->getPos()));

		Vec3* n2 = new Vec3(p1->getNormal().normalized());
		glNormal3fv((GLfloat *) n2);
		glVertex3fv((GLfloat *) &(p2->getPos()));

		Vec3* n3 = new Vec3(p1->getNormal().normalized());
